...
def batch_matrix_to_pycolmap(
    ...
    for fidx in range(N):
        ...
        # Create image first (new pycolmap API: id -> image_id)
        ...
        # Create frame and associate image data (new pycolmap API)
        frame = pycolmap.Frame()
        frame.frame_id = fidx
        frame.rig_id = 1
        frame.add_data_id(image.data_id)  # Associate image with frame
        reconstruction.add_frame(frame)

        # Set image pose through frame (new pycolmap API)
        ...
        # Update image with frame_id after frame is created
