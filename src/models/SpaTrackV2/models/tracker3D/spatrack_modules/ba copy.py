...
def log_ba_summary(summary):
    ...
    if summary.num_residuals_reduced > 0:
        ...
        # Safe computation with NaN checks
        initial_cost_per_res = summary.initial_cost / summary.num_residuals_reduced
        final_cost_per_res = summary.final_cost / summary.num_residuals_reduced

        if np.isnan(initial_cost_per_res) or initial_cost_per_res < 0:
            logging.warning(f"Initial cost : nan (invalid optimization) [px]")
        else:
            logging.info(f"Initial cost : {np.sqrt(initial_cost_per_res)} [px]")

        if np.isnan(final_cost_per_res) or final_cost_per_res < 0:
            logging.warning(f"Final cost : nan (invalid optimization) [px]")
        else:
            logging.info(f"Final cost : {np.sqrt(final_cost_per_res)} [px]")
